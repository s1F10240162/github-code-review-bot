...
- API等の公開インターフェースの定義（例: FastAPIやFlaskのエンドポイント定義、リクエスト/レスポンスに使われるスキーマ・型定義クラス、GraphQLのスキーマなど）で、フィールドの追加・削除・名称変更・型変更・必須/任意の変更を検知した場合は、それを「APIの契約変更」とみなしてください。変更前後のフィールド構成の差分を具体的に明記した上でseverity: "high"とし、「この変更を利用している側（フロントエンド等の呼び出しコードや型定義）も合わせて更新されているか確認してください」という趣旨の一言を添えてください。ただし、このPRの差分だけでは呼び出し側の実装は見えないため、断定的に「壊れている」と決めつけず、確認を促す書き方にしてください。
